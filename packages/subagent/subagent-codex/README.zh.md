...
Codex 子级会在一个全新的临时线程中，以单个轮次接收这些独立文本块。它的工作区是父会话 cwd；所选提供方实例会固定已配置的模型、环境、非交互审批策略与沙箱模式，而省略的模型及其余产品设置来自 Codex 原生配置。请求的角色设定以该轮次的首个文本块送达，因为 app-server 协议没有自定义系统提示字段——子级因此可能把它读作任务内容，而不是优先于任务的指令。可执行版本来自 Bundle 锁定的平台载荷。
...
- **可选共享能力中只支持 `persona`，且它不是系统指令**——角色设定以该轮次首个文本块的形式传入，因此模型可能把它当作任务内容。对于本提供方，共享服务仍会拒绝 `agentOptions`、输出 schema、工具筛选和 harness 深度强制约束；协议中存在 `turn/start.outputSchema`，但尚未接线。
